 ...
 * BrevoProvider
 *
 * Envio transacional via API REST oficial do Brevo (ex-Sendinblue).
...
class BrevoProvider implements EmailProviderInterface
{
    ...
    public function send(
        ...
        if (!empty($attachments)) {
            $payload['attachment'] = $this->buildAttachments($attachments);
        }
        ...
        email_error_log(
            ...
            'BrevoProvider::send ' . ($result['success'] ? 'OK' : 'FALHOU'),
    ...
    public function validateConfiguration(): array
    {
        ...
        if (empty($this->apiKey)) {
            $errors[] = 'API Key do Brevo é obrigatória';
        }
    ...
    private function request(string $url, array $payload): array
    {
        ...
        $error     = $success ? null : ($json['message'] ?? "HTTP $httpCode — $body");
...
<style>
  ...
  .hd{background:linear-gradient(135deg,#0ea5e9,#0369a1);color:#fff;padding:24px;text-align:center}
  .bd{padding:28px}
  .badge{display:inline-block;background:#dcfce7;color:#166534;padding:4px 12px;border-radius:20px;font-size:13px;margin-bottom:16px}
...
<body>
<div class="box">
  <div class="hd"><h2 style="margin:0">✅ Configuração Brevo OK</h2></div>
  <div class="bd">
    <div class="badge">API REST • Brevo (ex-Sendinblue)</div>
    <p>Sua integração com a <strong>API do Brevo</strong> está funcionando corretamente.</p>
    <table>
      <tr><td>Destinatário</td><td>$to</td></tr>
      <tr><td>Data/Hora</td><td>$date</td></tr>
      <tr><td>Provider</td><td>Brevo API v3</td></tr>
      <tr><td>Endpoint</td><td>api.brevo.com/v3/smtp/email</td></tr>
    </table>
  </div>
