 ...
 *   • Suporte a timeout personalizado por usuário (retornado pela API)
 ...
 *   • Apenas um fetch por requisição de sessão
 *   • Estado centralizado e compartilhado
 *   • Listeners são consumidores PASSIVOS
 *   • Logout centralizado (não espalhado em 24 páginas)
 *
 * ✅ RESPONSABILIDADES:
...
class SessionManagerCore {
    ...
    constructor() {
        if (SessionManagerCore.instance) {
            throw new Error('[SessionManager] ❌ SessionManagerCore já foi instanciado! Use getInstance().');
        }
    ...
    // ─── OBTER TOKEN BEARER ──────────────────────────────────────────────────
    /**
     * Lê o token do portal do morador salvo no localStorage após o login.
    ...
    /**
     * Monta os headers de Authorization para as requisições.
     * Se houver token do portal, usa Bearer. Caso contrário, sem header extra.
     */
    getAuthHeaders() {
    ...
    async initialize() {
        ...
        if (this.initializationPromise) {
            console.log('[SessionManager] ⏳ Inicialização já em andamento, aguardando...');
        ...
        this.initializationPromise = (async () => {
            ...
            // Detectar atividade do usuário para ping
            ...
            console.log('[SessionManager] ✅ Inicialização v3.0 concluída');
            return true;
        })();
    ...
    /**
     * Detecta atividade do usuário (mouse, teclado, toque) para
     * registrar que o usuário está ativo (evita expirar por inatividade).
     * O ping real é enviado a cada PING_INTERVAL (2min).
     */
    ...
    async checkSession() {
        if (this.isFetching) {
            console.log('[SessionManager] ℹ️ Verificação já em progresso, aguardando...');
        ...
        try {
            ...
            if (!response.ok) {
                console.warn('[SessionManager] ⚠️ Resposta não OK:', response.status);
            ...
            if (data.sucesso && data.sessao_ativa) {
                console.log('[SessionManager] ✅ Sessão ativa');
                ...
                // Emitir evento com todos os dados necessários para a UI
                ...
                console.warn('[SessionManager] ⚠️ Sessão inativa');
            ...
            if (error.name === 'AbortError') {
                console.warn('[SessionManager] ⚠️ Timeout na verificação (15s)');
                this.emit('error', { type: 'timeout', message: 'Servidor não respondeu em 15s' });
                this.isFetching = false;
                return this.isAuthenticated;
            }
     ...
     * Quando o servidor responde, o valor é sincronizado.
     */
    _startCountdown() {
        ...
        this.countdownTimer = setInterval(() => {
            if (this.countdownSeconds > 0) {
                ...
                // Também atualizar o evento userDataChanged para compatibilidade
                if (this.currentUser) {
                    this.emit('userDataChanged', {
                        user:          this.currentUser,
                        usuario:       this.currentUser,
                ...
                clearInterval(this.countdownTimer);
                this.countdownTimer = null;
                console.warn('[SessionManager] ⚠️ Countdown zerado — verificando sessão...');
                this.checkSession();
            }
        }, this.COUNTDOWN_TICK);
    }

    // ─── RENOVAR SESSÃO (ping de atividade) ──────────────────────────────────
    async renewSession() {
        ...
        // Só enviar ping se o usuário teve atividade recente (últimos 5 min)
        const inativo = Date.now() - (this._lastActivity || 0);
        if (inativo > 5 * 60 * 1000) {
            console.log('[SessionManager] ℹ️ Usuário inativo há ' + Math.round(inativo/60000) + 'min, pulando ping');
            return false;
        }
        ...
        try {
            ...
            console.warn('[SessionManager] ⚠️ Ping falhou:', response.status);
            this.isFetching = false;
            return false;
        } catch (error) {
    ...
    async logout() {
        ...
        try {
            ...
            }).catch(() => console.warn('[SessionManager] ⚠️ Erro ao chamar logout, mas continuando...'));
        } catch (e) {}
        ...
        } catch (e) { console.warn('[SessionManager] ⚠️ Erro ao limpar caches:', e); }
    ...
    startPeriodicChecks() {
        if (this.checkTimer) {
            console.log('[SessionManager] ℹ️ Verificações periódicas já ativas');
            return;
        }
        console.log('[SessionManager] ▶️ Iniciando verificações periódicas v3.0');

        // Verificação completa a cada 60s
    ...
    persistState() {
        ...
        } catch (e) { console.warn('[SessionManager] ⚠️ Erro ao persistir estado:', e.message); }
    }
...
(function initializeSessionManager() {
    ...
    if (document.readyState === 'loading') {
        ...
        try {
            ...
            console.error('[SessionManager] ❌ Erro na inicialização:', e);
        }
    }
})();
